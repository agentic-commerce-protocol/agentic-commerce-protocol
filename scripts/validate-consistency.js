...
const KNOWN_INLINE_EXAMPLE_FAILURES = new Set([
  ...
  // delegate_payment released: error response examples still wrapped in `{ error: {...} }`
  // in 2025-09-29, 2025-12-12, 2026-01-16, 2026-01-30, 2026-04-17. These validate cleanly
  // once unwrapped; the companion PR (released-version follow-up to #243) unwraps them and
  // removes these six entries. Allowlisted here so this validator can land independently.
  'delegate_payment|#/components/schemas/Error|invalid_request_missing_field',
  'delegate_payment|#/components/schemas/Error|idem_conflict',
  'delegate_payment|#/components/schemas/Error|invalid_request',
  'delegate_payment|#/components/schemas/Error|idempotency_key_required',
  'delegate_payment|#/components/schemas/Error|idempotency_in_flight',
  'delegate_payment|#/components/schemas/Error|idempotency_conflict'
]);
