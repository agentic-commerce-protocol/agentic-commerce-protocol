...
- **Spec**: `spec/unreleased/openapi/openapi.delegate_payment.yaml` and `spec/unreleased/json-schema/schema.delegate_payment.json` extended with `PaymentMethodCrypto` and oneOf payment_method. New error codes for x402: x402_invalid (malformed payload), x402_rejected (signature/nonce/amount/expiry), x402_network_unsupported, x402_settlement_failed.
