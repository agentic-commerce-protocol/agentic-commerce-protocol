...
- **Agentic checkout**: Shared `Address` in `schema.agentic_checkout.json` same optional `email`/`phone`.
- **OpenAPI**: `openapi.delegate_payment.yaml` and `openapi.agentic_checkout.yaml` updated to match the schema and RFC (Address, Allowance, DelegatePaymentResponse, PaymentMethodCard, Error; request/response examples).
