...
4. **Supporting diverse flows**: Handlers accommodate different integration patterns (delegated tokens, wallet credentials, agentic tokens)
...
**Handler Instance**: A seller's specific configuration of a handler, advertised in `payment.handlers[]`.
...
Each handler in `payment.handlers[]` follows this structure:
...
| `name` | string | ✅ | Handler specification name in reverse-DNS format (e.g., `dev.acp.tokenized.card`, `wallet.stripe.link`, `bnpl.klarna`). |
...
5. **Payment**: Agent submits payment instrument with SPT referencing handler
6. **Completion**: Seller processes payment using handler configuration
   {
     "payment_data": {
...
### 7.2 Base Pattern: `dev.acp.wallet`
...
**`requires_delegate_payment`**: `true` (wallet tokens wrapped in SPT for standardized allowance enforcement)
...
**PSP Declaration**:
- Wallet tokens contain PSP-specific routing configuration
- Agent vaults wallet token with Seller's PSP via `delegate_payment`
- SPT provides standardized wrapper with allowance constraints
- Allowance enforcement by Seller's PSP using SPT
...
**PSP Declaration**:
- Agent vaults BNPL auth tokens with Seller's PSP via `delegate_payment`
- SPT standardizes BNPL integration across different sellers
- BNPL provider issues authorization tokens that route to their infrastructure
...
```json
{
  ...
  "config": {
    ...
    "delegate_id": "delegate_1234567890",
    "delegate_display_name": "AI Shopping Assistant",
...
```json
{
  ...
  "config": {
    ...
    "psp": "stripe",
    "merchant_display_name": "Example Store",
    "delegate_id": "com.agent.payments",
    "delegate_display_name": "Shopping Assistant",
...
```json
{
  ...
  "config": {
    ...
    "psp": "stripe",
    "merchant_display_name": "Example Shop",
    "delegate_id": "delegate.shopping.assistant",
    "delegate_display_name": "AI Shopping Agent",
...
```json
{
  ...
  "config": {
    ...
    "delegate_id": "delegate.agent.platform",
    "delegate_display_name": "Checkout Agent",
    "api_version": 2,
    "api_version_minor": 0,
    "environment": "PRODUCTION",
    "merchant_info": {
...
This RFC introduces a **breaking change**: `seller_capabilities.payment_methods[]` is **removed** and **replaced** with `payment.handlers[]`.

**Before (simple payment_methods array):**
```json
{
  "payment": {
    "methods": ["card", "card.agentic_token", "wallet.stripe.link"]
  }
}
```
...
3. Replace `payment_methods[]` with `payment.handlers[]` in responses

**For Agents:**
1. Update session creation logic to parse `payment.handlers[]`
...
Specific handlers may require additional fields (e.g., `accepted_brands` for `dev.acp.tokenized.card`, `shop_id` for Shop Pay).
...
```json
{
  ...
  "description": "Schema for declaring payment handlers in payment.handlers",
  "type": "object",
  ...
  "properties": {
    ...
    "name": {
      ...
      "examples": ["dev.acp.tokenized.card", "wallet.stripe.link", "bnpl.klarna"]
    },
    "version": {
      "type": "string",
...
#### 10.3.3 Wallet Token Credential

**Schema URL**: `https://acp.dev/schemas/credentials/wallet_token.json`

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://acp.dev/schemas/credentials/wallet_token.json",
  "title": "Wallet Token Credential",
  "description": "Generic wallet token credential (specific wallets may extend)",
  "type": "object",
  "required": ["type", "token", "wallet_provider"],
  "properties": {
    "type": {
      "type": "string",
      "const": "wallet_token",
    ...
    "token": {
      "type": "string",
      "description": "Wallet-specific payment token",
      "minLength": 1
    },
    "wallet_provider": {
      "type": "string",
      "description": "Wallet provider identifier",
      "enum": ["stripe_link", "apple_pay", "google_pay", "shop_pay"],
      "examples": ["stripe_link", "apple_pay"]
    },
    "email": {
      "type": "string",
      "format": "email",
      "description": "Buyer email associated with wallet"
    }
  },
  "additionalProperties": true
}
...
#### 10.4.2 Stripe Link Handler Config Schema

**Schema URL**: `https://acp.dev/schemas/handlers/stripe_link/config.json`

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://acp.dev/schemas/handlers/stripe_link/config.json",
  "title": "Stripe Link Handler Configuration",
  "description": "Configuration schema for com.stripe.link handler",
  "type": "object",
  ...
  "properties": {
    ...
    "supported_payment_methods": {
      "type": "array",
      "description": "Payment methods supported via Link",
      "items": {
        "type": "string",
        "enum": ["card", "bank_account", "cash_app"]
      },
      "default": ["card"]
    },
    "environment": {
