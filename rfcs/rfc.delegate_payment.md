...
Two credential types are supported: **card** and **upi**.

| Field             | Type                                | Req | Description                                                    |
| ----------------- | ----------------------------------- | :-: | -------------------------------------------------------------- |
| `payment_method`  | PaymentMethodCard \| PaymentMethodUPI | ✅  | The credential to tokenize. (type MUST be `card` or `upi`.)   |
...
### 3.3 PaymentMethodCard (REQUIRED for card payments)
...
### 3.4 PaymentMethodUPI (REQUIRED for UPI payments)

- `type`: **MUST** equal `upi`.
- `delegate_id`: string (**REQUIRED**) — Unique identifier of the delegation set up by the user on an AI Agent (e.g., ChatGPT). All subsequent payments will use this delegate_id.
- `cryptogram`: string (**REQUIRED**) — Cryptogram provided for the delegated UPI payment. This is a one-time use cryptogram generated by the AI Commerce TSP (e.g., Razorpay) for each payment transaction.
- `metadata`: map<string,string> (OPTIONAL) — Additional metadata for the UPI payment method.

**UPI Payment Flow Context:**

UPI payments in the Agentic Commerce Protocol leverage **Delegated UPI** — a solution co-created by Razorpay & NPCI that enables AI agents to make payments on behalf of users with pre-authorized limits. The `delegate_id` represents the user's one-time setup of UPI delegation on their AI agent, and the `cryptogram` is fetched fresh for each transaction to ensure security.
...
- `payment_method.type` **MUST** be `card` or `upi`.
- `allowance.currency` matches `^[a-z]{3}$` (e.g., `usd`, `inr`).
- `allowance.expires_at` must be RFC 3339.
- At least one `risk_signal` item.

### 7.2 Card-Specific Validation Rules
...
### 7.3 UPI-Specific Validation Rules

- `payment_method.delegate_id` **MUST** be present (string).
- `payment_method.cryptogram` **MUST** be present (string).
- For UPI payments, `allowance.currency` is typically `inr` (Indian Rupees).
...
### 8.4 Success (UPI, one-time allowance)
...
```json
{
  ...
  "allowance": {
    ...
    "max_amount": 100000,
    "currency": "inr",
    "checkout_session_id": "csn_01HV3P3XYZ9DEF",
    "merchant_id": "swiggy_store",
    "expires_at": "2025-10-09T07:20:50.52Z"
  },
  "billing_address": {
    "name": "Rajesh Kumar",
    "line_one": "123 MG Road",
    "line_two": "Apartment 5B",
    "city": "Mumbai",
    "state": "Maharashtra",
    "country": "IN",
    "postal_code": "400001"
  ...
  "metadata": { "payment_method": "upi", "source": "chatgpt_checkout" }
}
```
...
```json
{
  "id": "vt_01J8Z3WXYZ9UPI",
  "created": "2025-10-01T11:00:00Z",
  "metadata": {
    "source": "agent_checkout",
    "merchant_id": "swiggy_store",
    "idempotency_key": "idem_upi_abc123",
    "payment_method": "upi"
  }
}
```

### 8.5 Error (invalid delegate_id for UPI)
...
```json
{
  ...
  "message": "Invalid or expired delegate ID provided",
...
- [ ] Accepts `API-Version` and validates `2025-09-29` or `2025-10-01`
- [ ] Verifies `Authorization` (Bearer)
- [ ] Validates request fields per §3 & §7
- [ ] Supports credential types: `card` and `upi`
...
- [ ] Redacts PCI/PII data in logs/telemetry (card numbers, cryptograms, etc.)

---

## 10. Change Log

- **2025-10-01**: Added support for **UPI** payment method via `PaymentMethodUPI`. Introduced `delegate_id` and `cryptogram` fields for delegated UPI payments. Updated validation rules and examples to cover both card and UPI flows.
